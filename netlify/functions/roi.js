...
// Netlify Function: 1y/3y/5y ROI (vs USD) for ZEC, BTC, gold (GC=F) and the
// S&P 500 (^GSPC) from Yahoo Finance's keyless chart API. Endpoint: /api/roi
  ...
  try {
    ...
    for (const [key, sym] of ROI_SYMS) {
      ...
      try {
        ...
        const r = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; ZcashJapan/1.0)' } });
